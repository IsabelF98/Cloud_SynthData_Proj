...
def pull_test_data(cell_sq_rad, stim_dim, scale=None):
    ...
    if scale is not None:
        out_file = 'data/cloud_TEST_data_stim_dim'+str(stim_dim)+'_spike_time_sqrad_'+str(cell_sq_rad)+'_sca='+str(scale)+'.hdf5'
    else:
        out_file = 'data/cloud_TEST_data_stim_dim'+str(stim_dim)+'_spike_time_sqrad_'+str(cell_sq_rad)+'.hdf5'
    ...
    for i in range(N_files):
        ...
        for j in range(len(cell_type_list)):
            ...
            for k in range(cell_NC):
                for l in range(NR):
                    if i == 0 and l == 0:
                        spike_time.append([cell_data[l][cell_idx[k]]])
                        spike_time[idx][l] = np.append(spike_time[idx][l],-1)
                    elif i == 0 and l != 0:
                        spike_time[idx].append(cell_data[l][cell_idx[k]])
                        spike_time[idx][l] = np.append(spike_time[idx][l],-1)
                    else:
                        spike_time[idx][l] = np.append(spike_time[idx][l],cell_data[l][cell_idx[k]])
                        spike_time[idx][l] = np.append(spike_time[idx][l],-1)
                idx += 1
    ...
    with h5py.File(out_file, 'w') as f:
        ...
        for i in range(len(spike_time)):
            for j in range(len(spike_time[i])):
                f.create_dataset('spike_time_cell_'+str(i)+'_repeat_'+str(j), data=spike_time[i][j])          
