...
class SimCloudData(Dataset):
    ...
    def __init__(self,
        ...
        if down_sample is not None:
            ...
            self.stim = np.round(
                downsample_stim(init_stim.reshape(self.NT,orig_L,orig_L), 
                                down_sample).reshape(self.NT,int(L*L))).astype(np.uint8)
        else:
        ...
        self.dfs = np.ones(self.robs.shape, dtype=np.uint8)
